...
int checkMandelbrot(float real, float imag, int cutoff){
	...
	*/
       int i = 0;
    float real_temp = real, real_first = real;
    float imag_temp = imag, imag_first = imag;

    while( i < cutoff ) {
        real = real_temp;
        imag = imag_temp;

        real_temp = real * real - imag * imag + real_first;
        imag_temp = 2 * real * imag + imag_first;

        
        if( (real_temp * real_temp + imag_temp * imag_temp) > 4 ) {
            return 0;
        }
	...
	return cutoff;                      //<-- This needs to be modified.
...
int main(int argc, char *argv[]){
	...
	*/
    ...
    for( int i = 0; i < total_ranks; i++) {  
        ...
        /* Blocks are scheduled using an Alternating Direction Scheme e.g. 3 Tasks
         * +---+---+---+
         * | 0 | 1 | 2 |
         * +---+---+---+
         * | 2 | 1 | 0 |
